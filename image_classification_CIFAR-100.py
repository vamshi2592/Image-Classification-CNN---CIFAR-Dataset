...
def test_one_hot_encode(one_hot_encode):
    ...
    test_numbers = np.random.choice(range(100), test_shape)
    one_hot_out = one_hot_encode(test_numbers)

    assert type(one_hot_out).__module__ == np.__name__,        'Not Numpy Object'

    assert one_hot_out.shape == (test_shape, 100),        'Incorrect Shape. {} shape found'.format(one_hot_out.shape)
...
def test_con_pool(conv2d_maxpool):
    ...
    assert conv2d_maxpool_out.get_shape().as_list() == [None, 4, 4, 100],        'Incorrect Shape.  Found {} shape'.format(conv2d_maxpool_out.get_shape().as_list())
...
def test_conv_net(conv_net):
    ...
    assert logits_out.get_shape().as_list() == [None, 100],        'Incorrect Model Output.  Found {}'.format(logits_out.get_shape().as_list())
