...
def test_normalize(normalize):
    test_shape = (np.random.choice(range(1000)), pixel, pixel, 3)
...
def test_one_hot_encode(one_hot_encode):
    ...
    test_numbers = np.random.choice(range(numberOfClasses), test_shape)
    one_hot_out = one_hot_encode(test_numbers)

    assert type(one_hot_out).__module__ == np.__name__,        'Not Numpy Object'

    assert one_hot_out.shape == (test_shape, numberOfClasses),        'Incorrect Shape. {} shape found'.format(one_hot_out.shape)
...
def test_con_pool(conv2d_maxpool):
    test_x = tf.placeholder(tf.float32, [None, pixel, pixel, 5])
    test_num_outputs = numberOfClasses
    test_con_k = (2, 2)
    test_con_s = (4, 4)
    test_pool_k = (2, 2)
    test_pool_s = (2, 2)
    ...
    assert conv2d_maxpool_out.get_shape().as_list() == [None, 4, 4, numberOfClasses],        'Incorrect Shape.  Found {} shape'.format(conv2d_maxpool_out.get_shape().as_list())
...
def test_conv_net(conv_net):
    test_x = tf.placeholder(tf.float32, [None, pixel, pixel, 3])
    test_k = tf.placeholder(tf.float32)

    logits_out = conv_net(test_x, test_k)

    assert logits_out.get_shape().as_list() == [None, numberOfClasses],        'Incorrect Model Output.  Found {}'.format(logits_out.get_shape().as_list())
...
def test_train_nn(train_neural_network):
    ...
    test_x = np.random.rand(128, pixel, pixel, 3)
    test_y = np.random.rand(128, numberOfClasses)
...
def process_and_save_imagesFolder(folderName, ImgMatrix, Imglabels, labelValue):
		...
		for filename in os.listdir(folderName):
			...
			newImg = newImg.resize((pixel, pixel), PIL.Image.ANTIALIAS)
			newImg = np.array(newImg)
			newImg = newImg.reshape(1, pixel, pixel, 3)
...
pathName = 'C://Anaconda3_500_ProgramFiles/envs/imageClassifyEnv/Images_dataset/'        
...
def load_cfar10_batch(cifar10_dataset_folder_path, batch_id):
    ...
    ImgMatrix = np.zeros(shape=(1, pixel, pixel, 3))
	
    Imglabels = [0]
	
    
    
    appleFolder = pathName+'train/Apple'
    bananaFolder = pathName+'train/Banana'
    orangeFolder = pathName+'train/Lemon'
    ...
    ImgMatrix, Imglabels = process_and_save_imagesFolder(orangeFolder, ImgMatrix, Imglabels, 3)
	
	
    #print('labels list for 100 classes: 'labels[20])
    print('testing matrix shape:::: ', ImgMatrix.shape)
    print(Imglabels)
    return ImgMatrix, Imglabels
...
def load_cfar10_batch_test(cifar10_dataset_folder_path, batch_id):
    ...
    appleFolder = pathName+'test/Apple'
    bananaFolder = pathName+'test/Banana'
    orangeFolder = pathName+'test/Lemon'
...
def preprocess_and_save_data(cifar10_dataset_folder_path, normalize, one_hot_encode):
    ...
    print('testing matrix shape features:::: ', features.shape)
...
def normalize(x):
    w = 0
    pixels = np.ndarray((len(x), pixel, pixel, 3))
    for p in x:
        p = p.flatten()
        p = abs((p - 128.) / 128.)
        p = p.reshape(1, pixel, pixel, 3)
        pixels[w, :, :, :] = p
        w += 1
    return pixels
...
def one_hot_encode(x):
    classes = list(range(numberOfClasses))
...
# Inputs
x = neural_net_image_input((pixel, pixel, 3))
y = neural_net_label_input(numberOfClasses)
